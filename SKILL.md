...
- **脚本生成器技能**: `../.trae/skills/ruijie-ebg-script-generator/SKILL.md`
- **红线问题文档**: `../zsk/红线问题.md`
- **日志接收目录**: `D:\MyCode\RJ-pytest-ebg\otherlog`
...
- 修复测试脚本、分析日志、终验、封包及推进下一个用例，均属于使用工作流，不代表授权修改工作流。
- 上述任务中，工作流目录、生成器技能及规范文档默认只读；禁止顺手修改 `workflow.py`、`SKILL.md` 或其他辅助工具。
- 测试脚本修正仅针对当前 `py/` 工作副本，不得修改 `bak/` 原件或无关用例。
- 遇到疑似 lint 误报，必须报告规则编号、命中代码和判断依据；不得自行修改、删除或放宽检查规则，也不得为了通过检查改坏正确的脚本逻辑。
- 疑似误报未解决时，可以继续完成不受影响的脚本修复，但必须明确说明检查阻塞项，不得宣称 lint 或终验已通过。
- 只有用户明确要求修改工作流、检查器或相关规范时，才允许修改对应文件，且不得扩大修改范围。
- 用户明确要求设置日志接收目录时，仅允许修改对应配置行，保留现有配置流程。
...
3. 初修脚本：
   ...
   - lint 报错先区分脚本问题与检查器疑似误报；后者按照“修改范围与工作流保护”处理，不得自动转为工作流维护任务。
   - 读取「脚本生成器技能」。
   - 读取「红线问题文档」。
